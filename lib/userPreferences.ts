...
export const getUserMailroom = async (
  userProfile: UserProfile | null
): Promise<string> => {
  ...
  useEffect(() => {
    const fetchPath = async () => {
      try {
        ...
        const defaultOrg = await getUserOrg(userProfile).catch(
          () => DEFAULT_ORG
        );
...
export const getMailroomDisplayName = async (
  mailroomSlug: string
): Promise<string | null> => {
...
export const getOrgDisplayName = async (
  orgSlug: string
): Promise<string | null> => {
  try {
    const { data, error } = await supabase.rpc(
      "get_organization_name_by_slug",
      {
