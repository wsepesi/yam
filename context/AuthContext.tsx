    ...
    try {
      ...
      if (data.user) {
        ...
        if (fetchedProfile && fetchedProfile.status === "REMOVED") {
          ...
          setUserProfile(null);
          return {
            error: new Error("Your access has been revoked."),
            success: false,
            userProfile: null,
          };
        }
...
export const withAuth = <P extends object>(
  ...
  const WithAuthComponent = (props: P): React.ReactElement | null => {
    ...
    const { org: orgSlugFromQuery, mailroom: mailroomSlugFromQuery } =
      router.query;
    const currentOrgSlug = Array.isArray(orgSlugFromQuery)
      ? orgSlugFromQuery[0]
      : orgSlugFromQuery;
    const currentMailroomSlug = Array.isArray(mailroomSlugFromQuery)
      ? mailroomSlugFromQuery[0]
      : mailroomSlugFromQuery;
    ...
    useEffect(() => {
      const verifyAccess = async () => {
        if (loadingTimedOut && (isLoading || isCheckingAccess)) {
          router.push(
            `/login?callbackUrl=${encodeURIComponent(router.asPath)}&reason=loading_timeout`
          );
          return;
        }
          ...
          if (
            ...
            if (currentOrgSlug && currentMailroomSlug) {
              ...
              try {
                ...
                const { data: orgProfileData, error: orgProfileError } =
                  await supabase
                    .from("organizations")
                    .select("slug")
                ...
                const {
                  data: mailroomProfileData,
                  error: mailroomProfileError,
                } = await supabase
                  .from("mailrooms")
                  .select("slug")
                  .eq("id", userProfile.mailroom_id)
                  .single();
              ...
              if (
                assignedOrgSlug !== currentOrgSlug ||
                assignedMailroomSlug !== currentMailroomSlug
              ) {
    ...
    const effectivelyLoading =
      isLoading || isCheckingAccess || (isAuthenticated && !userProfile);
