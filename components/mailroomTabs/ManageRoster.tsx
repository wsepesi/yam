...
import type { Resident } from "@/lib/types"; // Import Resident type
import type { MailroomTabProps } from "@/lib/types/MailroomTabProps"; // Import the new props type

import { AddResidentDialog } from "./AddResidentDialog";
import {
  type ResidentActionHandlers,
  residentColumns,
} from "./resident-columns";
...
export default function ManageRoster({
  ...
  const loadResidents = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/get-residents?orgSlug=${encodeURIComponent(orgSlug!)}&mailroomSlug=${encodeURIComponent(mailroomSlug!)}`,
        {
      ...
      setError(
        err instanceof Error ? err.message : "Could not load residents."
      );
      setResidents([]);
    } finally {
      setIsLoading(false);
      ...
      if (
        ...
        reader.onload = (e: ProgressEvent<FileReader>) => {
          ...
          try {
            ...
            const normalizedHeadersFromFile = headersFromFile.map((h) =>
            ...
            const missingHeaders = normalizedExpectedHeaders.filter(
              (expectedHeader) =>
                !normalizedHeadersFromFile.includes(expectedHeader)
            );
            if (missingHeaders.length > 0) {
              setFileError(
                `Missing required headers: ${missingHeaders.join(", ")}. Expected: ${expectedHeaders.join(", ")}. These should be the first 4 columns of the file.`
              );
              return;
            }

            const dataRows: ParsedResidentData[] = jsonDataRaw
              .slice(1)
              .map((rowArray) => {
                const typedRowArray = rowArray as unknown[];
                ...
                normalizedHeadersFromFile.forEach((header, index) => {
                  ...
                  const originalHeaderIndex =
                    normalizedExpectedHeaders.indexOf(header);
                  const key =
                    originalHeaderIndex !== -1
                      ? expectedHeaders[originalHeaderIndex]
                      : header;
                  row[key] = typedRowArray[index] as string; // Assuming all cell values can be treated as string for now
                });
                return row;
              })
              .filter(
            ...
            for (const row of dataRows) {
              ...
              if (residentId) {
                // Only check if resident_id is present
                if (firstResidentIdLength === null) {
                  firstResidentIdLength = residentId.length;
                } else if (residentId.length !== firstResidentIdLength) {
                  setFileError(
                    "Resident IDs have inconsistent lengths. This might indicate that leading zeros were dropped during export. Please ensure all Resident IDs are formatted as text and have the same number of digits."
                  );
                  return;
                }
              }
            ...
            setFileError(
              "Error processing file. Please ensure it's a valid CSV or XLSX."
            );
          }
        };
        reader.onerror = () => {
  ...
  const handleConfirmUpload = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          result.error || `Failed to upload roster. Status: ${response.status}`
        );
      }
    ...
    <div className="flex flex-col">
      ...
      {selectedFile && !isUploadModalOpen && (
        <div className="my-4 p-4 border border-gray-300 rounded-none bg-gray-50">
          <h3 className="font-medium text-sm text-[#471803]">
          ...
          {!fileError && (
            <p className="text-green-600 text-xs mt-1">
              File selected. Waiting for processing or replace by clicking
              &quot;Upload Roster&quot; again.
            </p>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-none shadow-xl w-full max-w-2xl">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium text-[#471803]">
            ...
            <p className="text-sm text-[#471803]/90 mb-2">
              Found {parsedData.length} residents in the file. Please review the
              first few entries below.
            </p>
            ...
            <div className="max-h-60 overflow-y-auto border border-[#471803]/20 rounded-none mb-4">
              <Table>
                  ...
                  <TableRow className="border-b-[#471803]/20">
                    {parsedData.length > 0 &&
                      Object.keys(parsedData[0]).map((key) => (
                        <TableHead
                          key={key}
                          className="text-[#471803]/70 text-xs px-2 py-2"
                        >
                          {key.replace("_", " ")}
                        </TableHead>
                      ))}
                ...
                <TableBody className="bg-white divide-y divide-[#471803]/10">
                  {parsedData
                    .slice(0, 5)
                    .map((row: ParsedResidentData, rowIndex) => (
                      <TableRow key={rowIndex} className="hover:bg-[#471803]/5">
                        {Object.values(row).map(
                          (
                            ...
                            <TableCell
                              key={cellIndex}
                              className="text-xs text-[#471803] py-2 px-2"
                            >
                              {String(value ?? "")}
                            </TableCell>
                          )
            ...
            {parsedData.length > 5 && (
              <p className="text-xs text-[#471803]/70 mb-4">
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-none shadow-xl w-full max-w-md">
            ...
            <p className="text-sm text-[#471803]/90 mb-4">
              Are you sure you want to remove {residentToRemove.first_name}{" "}
              {residentToRemove.last_name} ({residentToRemove.student_id}) from
              the roster?
            </p>
            <div className="flex justify-end space-x-2">
              <Button
      ...
      {isUploadWarningModalOpen && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white p-6 rounded-none shadow-xl w-full max-w-lg">
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-medium text-[#471803]">
                Upload Roster - Important Notice
              </h3>
              <button
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="flex items-start space-x-3 p-3 bg-yellow-50 border border-yellow-200 rounded-none">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-700">
                    Uploading a new roster will replace <strong>all</strong> of
                    the old residents. If you want to keep existing residents,
                    make sure they are present in the new roster, or upload new
                    residents individually using the &quot;Add Resident&quot;
                    button.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <h4 className="font-medium text-[#471803]">
                  Required File Format
                </h4>
                <div className="text-sm text-[#471803]/80 space-y-2">
                  <p>
                    <strong>File types:</strong> CSV (.csv) or Excel (.xlsx)
                  </p>
                  <p>
                    <strong>Required columns (first 4 columns):</strong>
                  </p>
                  ...
                  <p className="text-xs text-[#471803]/60 mt-2">
                    <strong>Note:</strong> Ensure resident IDs are formatted as
                    text to preserve leading zeros and maintain consistent
                    length.
                  </p>
                </div>
              </div>
      ...
      <div className="flex-1 space-y-4 pr-1">
        {" "}
        {/* Adjusted max-h for the new button section */}
        {error && (
          <div className="flex items-center space-x-2 p-2 bg-red-100 border border-red-400 text-red-700 text-xs rounded-none">
            ...
            <button
              ...
              className="ml-auto p-0.5 hover:bg-red-200 rounded-none"
            >
              <X size={14} />
            </button>
          </div>
        ...
        <div className="bg-white border border-[#471803]/20 rounded-none shadow-sm">
          <div className="flex justify-between items-center my-2 px-4 py-2">
            <h3 className="text-md font-medium text-[#471803]">
              Current Residents
            </h3>
            <div className="flex items-center gap-1">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="outline"
                    className="ml-1 border-[#471803]/50 text-[#471803] hover:bg-[#471803]/10 px-2 py-1.5 text-sm h-auto rounded-none"
                  >
                ...
                >
                  {table
                    ...
                    .map((column) => (
                      <DropdownMenuCheckboxItem
                        key={column.id}
                        className="capitalize text-xs text-[#471803]/90 hover:bg-[#471803]/10 focus:bg-[#471803]/10 px-6 py-1 rounded-none"
                      ...
                      >
                        {column.id.replace("_", " ")}{" "}
                        {/* Show column id with underscore replaced by space */}
                      </DropdownMenuCheckboxItem>
                    ))}
                </DropdownMenuContent>
              </DropdownMenu>
            ...
            <Table>
              <TableBody>
                {Array.from({ length: 5 }).map((_, i) => (
                  <SkeletonRow key={i} numberOfCells={5} />
                ))}
              </TableBody>
            </Table>
          )}
          {!isLoading && !error && residents.length === 0 && (
            <p className="text-[#471803]/70 italic text-sm px-4 py-2">
              No residents found.
            </p>
          )}
          {!isLoading && !error && residents.length > 0 && (
                ...
                <TableBody className="bg-white divide-y divide-[#471803]/10">
                  {table.getRowModel().rows?.length ? (
                    ...
                    <TableRow>
                      <TableCell
                        colSpan={5}
                        className="h-20 text-center text-[#471803]/70 text-sm"
                      >
                        No results.
                      </TableCell>
                    </TableRow>
