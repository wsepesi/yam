...
import {
  currentPackagesColumns,
  type PackageInfo,
  type RetrievedPackageInfo,
  retrievedPackagesColumns,
...
export default function ManagePackages() {
  ...
  const [retrievedColumnFilters, setRetrievedColumnFilters] =
    useState<ColumnFiltersState>([]);
  const [retrievedColumnVisibility, setRetrievedColumnVisibility] =
    useState<VisibilityState>({});
      ...
      try {
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Failed to load mailroom details."
        );
      }
    };
  ...
  const fetchCurrentPackages = async () => {
    ...
    try {
      const response = await fetch(
        `/api/packages/get-current?mailroomId=${mailroomId}`,
        {
          headers: { Authorization: `Bearer ${session.access_token}` },
        }
      );
  ...
  const fetchRetrievedPackages = async (initialLoad = false) => {
    ...
    try {
      const response = await fetch(
        `/api/packages/get-retrieved?mailroomId=${mailroomId}&limit=${limit}&offset=${offset}`,
        {
          headers: { Authorization: `Bearer ${session.access_token}` },
        }
      );
      const data = await response.json();
      if (!response.ok)
        throw new Error(data.error || "Failed to fetch retrieved packages");
      if (initialLoad) {
    ...
    try {
      ...
      const exportData = retrievedPackages.map((pkg) => ({
        ...
        "Created At": pkg.createdAt
          ? new Date(pkg.createdAt).toLocaleDateString()
          : "",
        "Retrieved Timestamp": pkg.retrievedTimestamp
          ? new Date(pkg.retrievedTimestamp).toLocaleDateString()
          : "",
      }));
      ...
      <div className="flex-1 overflow-y-auto space-y-4 pr-1 max-h-[60vh]">
            ...
            <Table>
              <TableBody>
                {Array.from({ length: 5 }).map((_, i) => (
                  <SkeletonRow
                    key={i}
                    numberOfCells={currentPackagesColumns.length}
                  />
                ))}
              </TableBody>
            </Table>
              ...
              <Table>
                ...
                <TableBody className="bg-white divide-y divide-[#471803]/10">
                  {currentTable.getRowModel().rows?.length ? (
                    ...
                    <TableRow>
                      <TableCell
                        colSpan={currentPackagesColumns.length}
        ...
        <div className="bg-white border border-[#471803]/20 rounded-none shadow-sm">
            ...
            <Table>
              <TableBody>
                {Array.from({ length: 5 }).map((_, i) => (
                  <SkeletonRow
                    key={i}
                    numberOfCells={retrievedPackagesColumns.length}
                  />
                ))}
              </TableBody>
            </Table>
          )}
          {!isLoadingRetrieved && !error && retrievedPackages.length === 0 && (
            <p className="text-[#471803]/70 italic text-sm px-4 py-2">
              No retrieved package history.
            </p>
          )}
                  ...
                  <TableBody className="bg-white divide-y divide-[#471803]/10">
                    {retrievedTable.getRowModel().rows?.length
                      ...
                      : !isLoadingRetrieved && (
                          <TableRow>
                            <TableCell
                              colSpan={retrievedPackagesColumns.length}
                              className="h-20 text-center text-[#471803]/70 text-sm"
                            >
                              No results.
                            </TableCell>
                    ...
                    {isLoadingRetrieved && retrievedPackages.length > 0 && (
                      <TableRow>
                        <TableCell
                          colSpan={retrievedPackagesColumns.length}
                          className="h-10 text-center text-[#471803]/70 italic text-sm py-2"
                        >
                          Loading more...
                        </TableCell>
                      </TableRow>
          ...
          <div className="flex items-center justify-between py-2 px-4">
            <div className="flex-1 text-xs text-[#471803]/70" />
            {retrievedPackages.length < totalRetrievedPackages &&
              !isLoadingRetrieved && (
                <Button
                  variant="outline"
                  size="sm"
