...
const OrgMailroomsTab: React.FC = () => {
  ...
  const orgSlug = Array.isArray(orgSlugFromQuery)
    ? orgSlugFromQuery[0]
    : orgSlugFromQuery;
  ...
  const [isCreateMailroomDialogOpen, setIsCreateMailroomDialogOpen] =
    useState(false);
  ...
  const fetchOrgDetailsAndMailrooms = async () => {
    ...
    if (!orgSlug || typeof orgSlug !== "string" || orgSlug.trim() === "") {
      ...
      console.error(
        "fetchOrgDetailsAndMailrooms: orgSlug is missing or invalid",
        orgSlug
      );
      setLoading(false);
      return;
    ...
    try {
      ...
      const orgDetailsResponse = await fetch(
        `/api/organizations/details?slug=${orgSlug}`,
        {
      ...
      if (!orgDetailsResponse.ok) {
        ...
        throw new Error(
          errorData.error ||
            `Failed to fetch organization details: ${orgDetailsResponse.status}`
        );
      }
      ...
      const statsResponse = await fetch(
        ...
        {
          // Use orgDetails.id if API needs ID
      ...
      if (!statsResponse.ok) {
        ...
        throw new Error(
          errorData.error ||
            `Failed to fetch mailroom data: ${statsResponse.status}`
        );
      }
  ...
  useEffect(() => {
    if (router.isReady && orgSlug && session) {
      // Ensure orgSlug and session are available
  ...
  const handleCreateNewMailroom = () => {
    if (!currentOrganizationId) {
      setError(
        "Organization details are not yet loaded or failed to load. Cannot create mailroom."
      );
  ...
  const getStatusPillClasses = (status: string): string => {
    const baseClasses =
      "inline-block px-2.5 py-1 text-xs font-semibold leading-tight"; // No rounded edges
          ...
          <table className="min-w-full divide-y divide-[#471803]/20">
            <thead>
              <tr className="bg-white">
                <th className="px-6 py-4 text-left text-xs font-medium text-[#471803]/70 uppercase tracking-wider">
                  Mailroom Name
                </th>
                <th className="px-6 py-4 text-left text-xs font-medium text-[#471803]/70 uppercase tracking-wider">
                  Status
              ...
              {loading ? (
                ...
                <tr>
                  <td
                    ...
                    className="px-6 py-10 text-center text-sm text-red-600 italic"
                  >
                    Error: {error}
                  </td>
                </tr>
                ...
                mailrooms.map((mailroom) => (
                  <tr key={mailroom.id} className="hover:bg-[#fffaf5]">
                    ...
                    <td className="px-6 py-5 whitespace-nowrap text-sm text-[#471803]">
                      {mailroom.packagesPending.toLocaleString()}
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap text-sm text-[#471803]">
                      {mailroom.totalUsers.toLocaleString()}
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap text-sm text-[#471803]">
                ...
                <tr>
                  <td
                    colSpan={visibleColumnCount}
                    className="px-6 py-10 text-center text-sm text-[#471803]/70 italic"
                  >
