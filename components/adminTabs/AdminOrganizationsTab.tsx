...
const AdminOrganizationsTab: React.FC = () => {
  ...
  const fetchOrganizations = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errorData.error || `Failed to fetch organizations: ${response.status}`
        );
      }
  ...
  const getStatusPillClasses = (status: string): string => {
    const baseClasses =
      "inline-block px-2.5 py-1 text-xs font-semibold leading-tight";
              ...
              {loading ? (
                ...
                organizations.map((org) => (
                  <tr key={org.id} className="hover:bg-[#fffaf5]">
                    ...
                    <td className="px-6 py-5 whitespace-nowrap text-sm text-[#471803]">
                      {org.totalMailrooms.toLocaleString()}
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap text-sm text-[#471803]">
                      {org.totalUsers.toLocaleString()}
                    </td>
                    <td className="px-6 py-5 whitespace-nowrap text-sm text-[#471803] font-mono">
                      {org.slug}
                    </td>
                  </tr>
                ))
