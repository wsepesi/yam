...
const Register: NextPage = () => {
  ...
  const [fetchedDbInvite, setFetchedDbInvite] =
    useState<DbInvitationRecord | null>(null);
  ...
  useEffect(() => {
    ...
    if (hash) {
      ...
      // Fallback to error=access_denied with a descriptive message
        ...
        (errorDescription
          .toLowerCase()
          .includes("link is invalid or has expired") ||
      ...
      if (isOtpError) {
        console.warn("Registration link error detected in URL hash:", {
          errorCode,
          errorParam,
          errorDescription,
        });
    ...
    const { data: authListenerData } = supabase.auth.onAuthStateChange(
      (event, session) => {
        console.log(
          ...
          event,
          session
            ? { userEmail: session.user?.email, eventType: event }
            : "No session"
        );
        ...
        if (event === "INITIAL_SESSION") {
          if (session?.user) {
            ...
            if (
              !knownNonResettableStates.includes(status) &&
            ...
            if (status === "checking_session") {
              console.warn(
                "INITIAL_SESSION: No user session found and no prior URL error. User likely navigated directly to /register or link was entirely invalid."
              );
              setError(
                "A valid, authenticated session is required to set your password. Please use the invitation link sent to your email. If you are sure you used the correct link, it might have expired or already been used."
              );
          ...
          setError(
            "Your session has ended. To set your password, please use the invitation link again."
          );
          setStatus("invalid_invite"); // Directly to invalid state as registration cannot proceed.
        } else if (event === "USER_UPDATED") {
          // User updated (e.g., password change).
          console.log(
            "User updated event received. Minimal handling in listener."
          );
        }
      }
    );
  ...
  useEffect(() => {
    const validateInvitation = async (user: User) => {
      ...
      if (!invitationId) {
        ...
        setError(
          "Your account is missing the required invitation details. Please contact support."
        );
        setStatus("invalid_invite");
        return;
      }
      ...
      try {
        ...
        if (user.email !== data.email) {
          console.warn(
            `Session email (${user.email}) does not match invitation email (${data.email})`
          );
          setError(
            "You are logged in with a different email than the one invited. Please use the correct account or contact support."
          );
          setStatus("invalid_invite");
          return;
        }
        ...
        if (data.status !== "PENDING") {
          ...
          const message =
            data.status === "RESOLVED" || data.status === "USED"
        ...
        setError(
          `Validation failed unexpectedly: ${err instanceof Error ? err.message : "Unknown error"}`
        );
  ...
  const onSubmit = async (formData: RegisterFormValues) => {
    if (!fetchedDbInvite || !sessionUser) {
      setError(
        "Cannot submit: Missing invitation details or user session. Please refresh."
      );
    ...
    if (sessionUser.email !== fetchedDbInvite.email) {
      setError(
        "Session email mismatch. Please ensure you are logged in with the invited email."
      );
    ...
    try {
      ...
      if (!currentUser?.id) {
        ...
        setError(
          "User session issue after password update. Please try logging in again."
        );
        setStatus("error");
        return;
      }
      ...
      console.log(
        `onSubmit: Updating invitation status to RESOLVED for ID: ${fetchedDbInvite.id} with user ID: ${currentUser.id}`
      );
      ...
      if (invitationUpdateError) {
        console.error(
          "onSubmit: Error updating invitation status to RESOLVED:",
          invitationUpdateError
        );
      } else {
        console.log(
          "onSubmit: Invitation status successfully updated to RESOLVED."
        );
      }

      console.log(
        `onSubmit: Attempting to update profile status to ACTIVE for user ID: ${currentUser.id}`
      );
      ...
      if (profileUpdateError) {
        console.error(
          "onSubmit: Error updating profile status to ACTIVE:",
          profileUpdateError
        );
        ...
        console.log(
          `onSubmit: Successfully updated profile status to ACTIVE for user: ${currentUser.id}`
        );
      ...
      if (!fetchedDbInvite.organizations?.slug) {
        console.error("onSubmit: Organization slug is missing.");
        setError(
          "Critical data missing for redirection (organization slug). Please contact support."
        );
        setStatus("error"); // Or 'ready_for_password' if you want to allow retry without re-fetching invite
        return;
      }

      if (!fetchedDbInvite.mailrooms?.slug) {
        console.error("onSubmit: Mailroom slug is missing.");
        setError(
          "Critical data missing for redirection (mailroom slug). Please contact support."
        );
      ...
      console.log(
        "onSubmit: Registration complete. Redirecting to dashboard..."
      );
      router.push(
        `/${fetchedDbInvite.organizations.slug}/${fetchedDbInvite.mailrooms.slug}/`
      );
    } catch (err) {
  ...
  if (
        ...
        <Layout title="Error | Yam" glassy={false}>
          <div className="flex flex-1 justify-center items-center h-full">
            <p className="text-red-600">
              Internal error: Missing invitation details in password state.
            </p>
          </div>
        </Layout>
      );
    ...
    const mailroomName =
      fetchedDbInvite.mailrooms?.name ||
      `Mailroom (${fetchedDbInvite.mailroom_id})`;
    const organizationName =
      fetchedDbInvite.organizations?.name ||
      `Organization (${fetchedDbInvite.organization_id})`;
      ...
      <Layout title="Complete Registration | Yam" glassy={false}>
        <div className="flex flex-1 justify-center items-center h-full">
          <div className="w-full max-w-md p-8 space-y-6 bg-[#ffeedd]">
            ...
            {error &&
              (status === "ready_for_password" || status === "error") && (
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <div className="space-y-1">
                <Label htmlFor="password" className="text-[#471803]/90">
                  Create Password
                </Label>
                <Input
                  id="password"
              ...
              <div className="space-y-1">
                <Label htmlFor="confirmPassword" className="text-[#471803]/90">
                  Confirm Password
                </Label>
                <Input
                  id="confirmPassword"
  ...
  return (
    <Layout title="Registration | Yam" glassy={false}>
      <div className="flex flex-1 justify-center items-center h-full">
        <p className="text-[#471803]">
          Loading registration page or encountered an unexpected state.
        </p>
      </div>
    </Layout>
  );
