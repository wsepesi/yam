...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<{ packages: ApiRetrievedPackage[], totalCount?: number } | { error: string }>
) {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    const { data: packagesData, error: packagesError, count } = await query.returns<SupabaseRetrievedPackage[]>();

    if (packagesError) {
      console.error('Error fetching retrieved packages:', packagesError);
      return res.status(500).json({ error: 'Failed to fetch retrieved packages' });
    }
    ...
    const formattedPackages: ApiRetrievedPackage[] = packagesData.map((pkg) => ({
      id: pkg.id,
    ...
    return res.status(200).json({ packages: formattedPackages, totalCount: count ?? 0 });
