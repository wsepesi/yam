...
export default async function handler(
  req: NextApiRequest,
  res: NextApiResponse<{ message: string; counts?: { total: number; new: number; unchanged: number; updated: number; removed: number } } | { error: string }>
) {
  if (req.method !== 'POST') {
  ...
  try {
    ...
    if (fetchError) {
      ...
      return res.status(500).json({ error: 'Failed to fetch existing residents.' });
    }

    // Helper function to create a lookup key for residents
    const createLookupKey = (studentId: string, email: string | null | undefined) => {
    ...
    const findMatchingResident = (newResident: UploadedResident) => {
      ...
      const exactKey = createLookupKey(newResident.resident_id!, newResident.email);
    ...
    const hasDataChanged = (existing: { first_name: string; last_name: string; email: string | null }, newResident: UploadedResident) => {
      return existing.first_name !== newResident.first_name ||
    ...
    for (const residentUnchanged of residentsUnchanged) {
      ...
      if (updateError) {
        console.error('Error updating timestamp for unchanged resident:', updateError);
        return res.status(500).json({ error: 'Failed to update unchanged residents.' });
      }
    }

    // Set non-matching existing residents to REMOVED_BULK
    const residentsToRemove = existingResidents?.filter(resident => 
      !matchedExistingIds.has(resident.id)
    ) || [];
    ...
    if (residentsToRemove.length > 0) {
      ...
      if (removeError) {
        ...
        return res.status(500).json({ error: 'Failed to remove old residents.' });
      }
    }
