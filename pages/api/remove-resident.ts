...
export default async function handler(
  ...
  if (!residentId) {
    return res.status(400).json({ error: 'Missing required data: residentId.' });
  }
  ...
  try {
    ...
    if (updateError) {
      ...
      return res.status(500).json({ error: `Failed to remove resident: ${updateError.message}` });
    }

    return res.status(200).json({ message: `Resident ${resident.first_name} ${resident.last_name} successfully removed.` });
