...
export default async function handler(
  ...
  if (!resident || !orgSlug || !mailroomSlug) {
    return res.status(400).json({ error: 'Missing required data: resident, orgSlug, or mailroomSlug.' });
  }

  // Validate required fields
  if (!resident.first_name || !resident.last_name || !resident.resident_id) {
    return res.status(400).json({ error: 'Missing required resident fields: first_name, last_name, or resident_id.' });
  }
  ...
  try {
    ...
    if (checkError) {
      ...
      return res.status(500).json({ error: 'Failed to check for existing resident.' });
    }

    if (existingResident) {
      return res.status(400).json({ error: `A resident with ID ${resident.resident_id} is already active in this mailroom.` });
    }
    ...
    if (insertError) {
      ...
      return res.status(500).json({ error: `Failed to add resident: ${insertError.message}` });
    }

    return res.status(200).json({ message: `Resident ${resident.first_name} ${resident.last_name} successfully added.` });
