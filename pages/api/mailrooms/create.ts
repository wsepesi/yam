...
export default async function handler(
  ...
  if (!name || !slug || !organizationId || !adminEmail) {
    return res.status(400).json({
      error: "Missing required fields: name, slug, or organizationId",
    });
  }
  ...
  try {
    ...
    if (profileError || !userProfile) {
      ...
      return res
        .status(400)
        .json({ error: "Could not fetch user profile to verify permissions." });
    }
    ...
    if (!canCreateMailroom) {
      return res.status(403).json({
        error:
          "User does not have permission to create a mailroom for this organization.",
      });
    }
    ...
    if (createError) {
      ...
      return res.status(500).json({
        error: createError.message || "Failed to create mailroom in database.",
      });
    }
    ...
    if (!newMailroom) {
      ...
      return res.status(500).json({
        error: "Mailroom created but no data was returned from database.",
      });
    }
