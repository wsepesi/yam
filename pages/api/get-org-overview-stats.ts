...
const getMonthYearStrings = (
  numMonths: number
): { name: string; isoMonth: string }[] => {
...
export default async function handler(
  ...
  try {
    ...
    if (!userId) {
      // getUserId might return null or throw, handle null case
    ...
    if (!orgSlug || typeof orgSlug !== "string") {
      return res.status(400).json({
        error:
          "Organization slug is required and must be a string in query parameters",
      });
    }
    ...
    if (orgError)
      throw new Error(
        `Failed to fetch organization details: ${orgError.message}`
      );
    if (!orgData)
      throw new Error(
        `Organization not found for the provided slug: ${orgSlug}`
      );
    ...
    if (orgUserError) {
      ...
      return res
        .status(500)
        .json({ error: "Error checking organization membership" });
    }
    if (orgUserCount === 0) {
    ...
    if (mailroomsError)
      throw new Error(`Failed to fetch mailrooms: ${mailroomsError.message}`);
    if (!mailroomsData)
      throw new Error("No mailrooms found for this organization.");
    ...
    if (allPackagesError)
      throw new Error(
        `Failed to fetch packages for mailrooms: ${allPackagesError.message}`
      );
    ...
    if (allProfilesError)
      throw new Error(
        `Failed to fetch profiles for mailrooms: ${allProfilesError.message}`
      );
    ...
    if (allResidentsError)
      throw new Error(
        `Failed to fetch residents for mailrooms: ${allResidentsError.message}`
      );

    for (const mr of mailroomsData as MailroomCoreData[]) {
      const packagesForMailroom =
        allPackages?.filter((p) => p.mailroom_id === mr.id) || [];
      const profilesForMailroom =
        allProfiles?.filter((p) => p.mailroom_id === mr.id) || [];
      const residentsForMailroom =
        allResidents?.filter((r) => r.mailroom_id === mr.id) || [];

      const totalPackages = packagesForMailroom.length;

      const totalResidents = residentsForMailroom.length;

      const packagesAwaitingPickup = packagesForMailroom.filter(
        (p) => p.status === "WAITING"
      ).length;
    ...
    const monthlyChartData: MonthlyChartDataPoint[] = monthYearStrings.map(
      (m) => ({
        name: m.name,
        total: 0,
        ...mailroomsData.reduce(
          (
            acc: { [key: string]: number },
            currentMailroom: MailroomCoreData
          ) => {
    ...
    const { data: monthlyPackagesRaw, error: monthlyPackagesError } =
      await supabaseAdmin
        .from("packages")
        ...
        .in(
          "mailroom_id",
          mailroomsData.map(
            (currentMailroom: MailroomCoreData) => currentMailroom.id
          )
        )
    ...
    if (monthlyPackagesError)
      throw new Error(
        `Failed to fetch monthly package data: ${monthlyPackagesError.message}`
      );
    ...
    if (monthlyPackagesRaw) {
      for (const pkg of monthlyPackagesRaw as PackageTimestamp[]) {
        ...
        const chartPoint = monthlyChartData.find((cd) =>
          monthYearStrings.find(
            (mys) => mys.isoMonth === pkgMonthISO && mys.name === cd.name
          )
        );
        if (chartPoint && pkg.mailroom_id) {
          (chartPoint[pkg.mailroom_id] as number) =
            ((chartPoint[pkg.mailroom_id] as number) || 0) + 1;
