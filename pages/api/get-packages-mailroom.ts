...
export default async function handler(
  ...
  if (
    ...
    return res.status(400).json({
      error: "orgSlug and mailroomSlug are required query parameters.",
    });
  }
  ...
  try {
    ...
    if (mailroomError || !mailroomData) {
      ...
      return res
        .status(404)
        .json({ error: "Mailroom not found or not unique." });
    }
    ...
    const [
      ...
      supabaseAdmin.rpc("get_monthly_package_stats_for_mailroom", {
    ...
    if (awaitingResult.error) {
      console.error(
        "Error fetching awaiting pickup count:",
        awaitingResult.error
      );
      return res
        .status(500)
        .json({ error: "Failed to fetch awaiting pickup count" });
    }
    const awaitingPickupCount = awaitingResult.count;

    if (awaitingTodayResult.error) {
      console.error(
        "Error fetching awaiting today count:",
        awaitingTodayResult.error
      );
      return res
        .status(500)
        .json({ error: "Failed to fetch new packages today count" });
    }
    const awaitingTodayCount = awaitingTodayResult.count;

    if (monthlyStatsResult.error) {
      console.error(
        "Error fetching monthly package stats:",
        monthlyStatsResult.error
      );
      return res
        .status(500)
        .json({ error: "Failed to fetch monthly package statistics" });
    }
    ...
    if (monthlyPackageStats && Array.isArray(monthlyPackageStats)) {
      for (const row of monthlyPackageStats) {
        ...
        const increase =
          packageVolumeMonthly.length > 0 ||
          monthlyPackageStats.indexOf(row) > 0
            ? currentPackages - lastMonthPackages
            : 0;
    ...
    const latestMonthData =
      packageVolumeMonthly.length > 0
        ? packageVolumeMonthly[packageVolumeMonthly.length - 1]
        : { name: "", packages: 0, increase: 0 };
