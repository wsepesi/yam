...
export default async function handler(
  ...
  try {
    ...
    let queryBuilder = supabaseAdmin
      .from("residents")
      .select(
        "id, mailroom_id, first_name, last_name, student_id, email, created_at, updated_at, added_by, status"
      )
    ...
    if (searchQuery && typeof searchQuery === "string") {
      ...
      queryBuilder = queryBuilder.or(
        `student_id.ilike.%${searchQuery}%,first_name.ilike.%${searchQuery}%,last_name.ilike.%${searchQuery}%`
      );
    }
