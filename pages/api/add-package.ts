...
export default async function handler(
  ...
  try {
    const { orgSlug, mailroomSlug, ...packageData } =
      req.body as PackageNoIds & { orgSlug: string; mailroomSlug: string };

    if (
      !packageData?.residentId ||
      !packageData.provider ||
      !orgSlug ||
      !mailroomSlug
    ) {
      return res.status(400).json({
        error: "Missing required package data, orgSlug, or mailroomSlug",
      });
    }
    ...
    if (!userId) {
      // Ensure userId is fetched for logging
      return res.status(401).json({
        error: "Unauthorized or unable to determine staff ID for logging.",
      });
    }
    ...
    const { data: mailroomRecord, error: mailroomError } = await supabaseAdmin
      .from("mailrooms")
      .select(
        "id, organization_id, admin_email, mailroom_hours, email_additional_text"
      ) // Select fields needed later
    ...
    try {
      const { data: packageNumber, error: queueError } =
        await supabaseAdmin.rpc("get_next_package_number", {
          p_mailroom_id: mailroomId,
        });

      if (queueError || !packageNumber) {
        throw new Error(
          `Failed to get package number: ${queueError?.message || "No package numbers available"}`
        );
      }
      ...
      if (!existingResident) {
        throw new Error(
          `No resident found with student ID ${packageData.residentId} in this mailroom`
        );
      }
      ...
      if (!mailroomRecord.admin_email) {
        throw new Error(
          "Admin email not configured for the mailroom. Cannot send notification."
        );
      }
      ...
      if (
        mailroomRecord.mailroom_hours &&
        typeof mailroomRecord.mailroom_hours === "object"
      ) {
        const hours = mailroomRecord.mailroom_hours as Record<
          string,
          { closed: boolean; periods: Array<{ open: string; close: string }> }
        >;
        const lines: string[] = [];
        Object.entries(hours).forEach(([day, schedule]) => {
          if (schedule.closed) {
            ...
            const periodStrings = schedule.periods.map(
              (p) => `${p.open} - ${p.close}`
            );
      ...
      fetch(
        `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/api/send-notification-email`,
        {
          method: "POST",
          headers: {
