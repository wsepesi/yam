  ...
  try {
    ...
    if (userProfile.role !== "super-admin") {
      return res.status(403).json({
        error:
          "User does not have permission to access system overview statistics.",
      });
    }

    // --- Fetch Real Data ---
    const totalOrganizations = await supabaseAdmin
      .from("organizations")
    ...
    const totalUsers = await supabaseAdmin
      ...
      .select("id", { count: "exact", head: true })
      .then((r) => r.count || 0);
    const totalMailrooms = await supabaseAdmin
      .from("mailrooms")
      .select("id", { count: "exact", head: true })
      .then((r) => r.count || 0);

    const { count: overallTotalPackages, error: packagesError } =
      await supabaseAdmin
        .from("packages")
        .select("id", { count: "exact", head: true });
    ...
    if (packagesError) {
      ...
      return res.status(500).json({
        error: `Failed to fetch total packages: ${packagesError.message}`,
      });
    }
    ...
    const initialMonthlyChartData: SystemMonthlyChartDataPoint[] =
      monthYearStrings.map((m) => ({
        name: m.name,
        totalPackages: 0,
      }));
    ...
    if (monthlyPackagesError) {
      console.error(
        "Error fetching monthly package data:",
        monthlyPackagesError
      );
      // Decide if this is critical or if you can proceed with empty chart data
      return res.status(500).json({
        error: `Failed to fetch monthly package data: ${monthlyPackagesError.message}`,
      });
    }

    const finalMonthlyChartData = initialMonthlyChartData.reduce(
      (acc, monthData) => {
        const monthPackages =
          monthlyPackagesRaw?.filter((pkg) => {
            const pkgDate = new Date(pkg.created_at);
            return (
              `${pkgDate.getFullYear()}-${(pkgDate.getMonth() + 1).toString().padStart(2, "0")}` ===
              monthYearStrings.find((m) => m.name === monthData.name)?.isoMonth
            );
          }).length || 0;
