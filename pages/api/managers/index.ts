...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (userProfile.role !== 'manager' && userProfile.role !== 'admin') {
      return res.status(403).json({ error: 'Only managers and admins can view managers' });
    }
    ...
    if (userProfile.role !== 'admin' && userProfile.organization_id !== mailroom.organization_id) {
      return res.status(403).json({ error: 'You can only view managers in your organization' });
    }
