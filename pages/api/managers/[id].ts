...
export default async function handler(
  ...
  try {
    ...
    if (!id || !role) {
      return res
        .status(400)
        .json({ error: "Missing required parameters (id, role)" });
    }
    ...
    if (role !== "user" && role !== "manager" && role !== "admin") {
      return res.status(400).json({
        error:
          "Invalid target role value. Can only assign user, manager, or admin.",
      });
    }
    ...
    if (
      ...
      return res.status(403).json({
        error: "Only super-admins, admins, and managers can update roles",
      });
    }
    ...
    if (userProfile.role === "manager") {
      ...
      if (userProfile.organization_id !== managerProfile.organization_id) {
        return res.status(403).json({
          error: "Managers can only update users in their own organization",
        });
      }
      ...
      if (
        ...
        managerProfile.role === "super-admin"
      ) {
        return res.status(403).json({
          error: "Managers cannot promote to admin or modify an admin's role",
        });
      }
    } else if (
      userProfile.role === "admin" ||
      ...
      if (managerProfile.role === "super-admin") {
        // Target is super-admin
        return res
          .status(403)
          .json({ error: "Admins cannot modify a super-admin's role." });
      }
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "Only admins or super-admins can promote to admin" });
    }
    ...
    if (updateError) {
      ...
      return res
        .status(500)
        .json({ error: "Failed to update manager profile" });
    }
    ...
    if (status === "REMOVED") {
      ...
      if (deleteError) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to delete user from auth table" });
      }
    }
