...
export default async function handler(
  ...
  if (!packageId || !orgSlug || !mailroomSlug) {
    return res.status(400).json({
      error:
        "packageId, orgSlug, and mailroomSlug are required in the request body.",
    });
  }
  ...
  try {
    ...
    if (packageError || !packageData) {
      return res.status(404).json({
        error: `Package not found or already retrieved: ${packageError?.message}`,
      });
    }
    ...
    if (updateError) {
      return res.status(500).json({
        error: `Failed to update package status: ${updateError.message}`,
      });
    }
    ...
    if (releaseError || !release) {
      return res.status(500).json({
        error: `Failed to release package number: ${releaseError?.message}`,
      });
    }
