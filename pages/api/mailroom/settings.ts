  ...
  try {
    ...
    if (
      ...
      return res.status(403).json({
        error: "Insufficient permissions to view settings for this mailroom.",
      });
    }
    ...
    if (error) {
      if (error.code === "PGRST116") {
        return res
          .status(404)
          .json({ error: "No email settings found for this mailroom." });
      }
    ...
    if (!data) {
      return res
        .status(404)
        .json({ error: "No email settings found (data is null)." });
    }
