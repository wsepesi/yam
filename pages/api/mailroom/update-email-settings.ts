...
export default async function handler(
  ...
  try {
    ...
    if (profileError || !profileData) {
      ...
      return res
        .status(401)
        .json({ error: "Unauthorized or profile not found." });
    }
    ...
    if (
      !(
        ...
        (profileData.role === "manager" &&
          profileData.mailroom_id === mailroomId)
      )
    ) {
      return res.status(403).json({
        error: "Insufficient permissions to update settings for this mailroom.",
      });
    }
    ...
    if (mailroomHours !== undefined)
      updateObject.mailroom_hours = mailroomHours;
    if (emailAdditionalText !== undefined)
      updateObject.email_additional_text = emailAdditionalText;
    ...
    if (updateError) {
      ...
      return res.status(500).json({
        error: `Failed to update email settings: ${updateError.message}`,
      });
    }

    return res
      .status(200)
      .json({ success: true, message: "Email settings updated successfully." });
  } catch (error) {
