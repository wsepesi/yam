  ...
  try {
    ...
    if (
      ...
      return res.status(403).json({
        error: "Only super-admins, admins, and managers can delete invitations",
      });
    }
    ...
    if (
      userProfile.role !== "super-admin" &&
      userProfile.role !== "admin" &&
      userProfile.organization_id !== invitation.organization_id
    ) {
      return res.status(403).json({
        error: "Managers can only delete invitations in their organization",
      });
    }
    ...
    if (
      ...
      return res
        .status(403)
        .json({ error: "Managers can only delete invitations they created" });
    }
