...
export default async function handler(
  ...
  try {
    const { orgSlug, mailroomSlug, ...packageData } = req.body as Package & {
      orgSlug: string;
      mailroomSlug: string;
    };

    if (!packageData?.packageId || !orgSlug || !mailroomSlug) {
      return res.status(400).json({
        error: "Package data, orgSlug, and mailroomSlug are required",
      });
    }
    ...
    if (orgError || !orgData) {
      console.error(
        `Error fetching organization by slug ${orgSlug} or mailroom mismatch for logging:`,
        orgError
      );
      return res.status(404).json({
        error:
          "Organization not found or mailroom does not belong to it for logging.",
      });
    }
    ...
    if (packageError || !packageRecord) {
      return res
        .status(404)
        .json({ error: "Package not found or not marked as retrieved" });
    }

    // Convert the package data to the LogPackage format
    const pickedUpAt =
      packageRecord.retrieved_timestamp || new Date().toISOString();
    ...
    const errorMessage =
      error instanceof Error ? error.message : "Failed to log package";
